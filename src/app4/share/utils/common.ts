...
//export const unique = (xs: Array<any>) => [...new Set(xs)];
//export const uniqueBy = (f = id) => (xs: Array<any>) => [...new Map(xs.map(x => [f(x), x])).values()];

export const groupBy = (f = id) => (xs: Array<any>) => xs.reduce((a, x) => {
    let key = f(x);
    a[key] = a[key] || [];
    a[key].push(x);
    return a;
