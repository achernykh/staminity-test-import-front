export let translateDialogs = {
    ru: {
        yes: 'Да',
        no: 'Нет',
        ok: 'Принять',
        cancel: 'Отклонить',
        performActionA: 'Принять запрос',
        performActionD: 'Отклонить запрос',
        performActionC: 'Отменить запрос',
        confirm: 'Вы уверены?',

        coaches: 'Тренеры',
        athletes: 'Спортсмены',
        friends: 'Друзья',
        following: 'Подписки',
        followers: 'Подписчики',
        members: 'Участники',

        bySelf: 'За свой счет',
        byClub: 'За счет клуба',
        byCoach: 'За счет тренера',
        tariffs: 'Тарифы',
        Premium: 'Премиум',
        Coach: 'Тренер',
        ClubAthletes: 'Спортсмен',
        ClubCoach: 'Тренер',
        ClubCoaches: 'Тренер',
        ClubManagement: 'Администратор',
        turnOffAthletes: 'Отключить атлетов?',

        startCoach: 'Отправить запрос тренеру?',
        rejectRequest: 'Отменить заявку?',
        leaveCoach: 'Покинуть тренера?',
        startFriends: 'Отправить заявку на добавление в друзья?',
        startFollow: 'Подписаться?',
        startClub: 'Отправить заявку на вступление в клуб?',
        leaveClub: 'Покинуть клуб?',
        excludeClub: 'Исключить из клуба?',

        // Операции с тренировками
        deleteActualActivity: 'Вы хотите удалить выбранные тренировки? Отменить операцию невозможно. Если удалить выполненные тренировки, они не будут импортированы при новой загрузке данных',
        deletePlanActivity: 'Вы хотите удалить выбранные тренировки? Отменить операцию невозможно',
        moveActualActivity: 'Перемещение тренировок с фактическими данными не возможно. Хотите скопировать только плановые данные?',
        updateIntensity: 'Тренировочные зоны у атлетов могут отличаться. Хотите пересчитать плановые значения интенсивности тренировки в соответсвии с зонами атлета?',

        // окно загрузки профиля
        upload: 'Загрузить',
        choose: 'Выбрать...',
        uploadFile: 'Загрузка файлов'


    },
    en: {

    }
};