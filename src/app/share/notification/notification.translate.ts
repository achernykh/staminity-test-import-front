...
/* ---------- Оповещения в клубе  --------**/
    ...
    **/
     ...
     **/
        ...
        trialExpireInSomeDays: 'Пробный период по тарифу {tariff} подходит к концу',
        trialExpireToday: 'Пробный период по тарифу {tariff} закончится сегодня',
        trialTariffExpired: 'Закончился пробный период по тарифу {tariff}',
        trialExpireInSomeDaysAfterOldTariff: 'Завершается срок пробного использоания по тарифу ... С ДД.ММ.ГГГГ тариф будет подключен на платной основе, начисления будут производиться ежедневно в рамках существующего счета ',
        trialExpireTodayAfterOldTariff: 'Сегодня завершается срок пробного использоания по тарифу ... С ДД.ММ.ГГГГ тариф будет подключен на платной основе, начисления будут производиться ежедневно в рамках существующего счета ',
        trialTariffExpiredAfterOldTariff: 'Закончился срок пробного использоания по тарифу ... С сегодняшнего дня тариф подключен на платной основе, начисления производятся ежедневно в рамках существующего счета ',
        billPayment_singleTariff: 'Вы оплатили счет на сумму ..... Тариф {tariffCode} продлен до ...дд.мм.гггг',
        billPayment_multiTariff: 'Вы оплатили счет на сумму ..... Срок действия по подключенным тарифам продлен до ...дд.мм.гггг',
        billPayment_singleTariff_ZeroAmount: 'Дополнительных начислений по тарифу {tariffCode} нет. Тариф продлен до ...дд.мм.гггг ',
        billPayment_multiTariff_ZeroAmount: 'Дополнительных начислений по вашим тарифам нет. Тарифы продлены до ...дд.мм.гггг ',
        billPayment_Failure_singleTariff: 'Не удалось оплатить счет по тарифу ... Оплатите счет вручную',
        billPayment_Failure_multiTariff: 'Не удалось оплатить счет по вашим тарифам. Оплатите счет вручную',
        billExpiredPaymentReceived: 'Спасибо! Ваш платеж по счету ... на сумму ... получен',
        disabledPremiumByCoach: 'Вам временно отключен тариф Премиум, ожидается оплата от тренера',
        disabledPremiumByClub: 'Вам временно отключен тариф Премиум, ожидается оплата от клуба',
        disabledCoachByClub: 'Вам временно отключен тариф Тренер, подключенный клубом. Ожидается оплата от клуба',
        tariffRecurringTonight: 'Приближается срок оплаты по тарифу {tariff}. ДД.ММ. ГГГГ будет проведена попытка списания сумма (валюта)',
        tariffRecurringTonight: 'Приближается срок оплаты по тарифу {tariff}. ДД.ММ. ГГГГ будет проведена попытка списания сумма (валюта)',
        tariffRecurringTonight: 'Приближается срок оплаты по тарифу {tariff}. ДД.ММ. ГГГГ будет проведена попытка списания сумма (валюта)',
        tariffRecurringTonightMulti: 'Приближается срок оплаты по подключенным тарифам. ДД.ММ. ГГГГ будет проведена попытка списания сумма (валюта)',
        tariffRecurringSuccess: 'Оплата по тарифу Премиум выполнена успешно. С вашей карты списано сумма (валюта). Тариф Премиум продлен до ...дд.мм.гггг',
        tariffRecurringSuccess: 'Оплата по тарифу Тренер выполнена успешно. С вашей карты списано сумма (валюта). Тариф Тренер продлен до ...дд.мм.гггг',
        tariffRecurringSuccess: 'Оплата по тарифу Клуб выполнена успешно. С вашей карты списано сумма (валюта). Тариф Клуб продлен до ...дд.мм.гггг',
        tariffRecurringSuccessMulti: 'Оплата за продление срока действия по подключенным тарифам выполнена успешно. С вашей карты списано сумма (валюта). Тарифы продлены до ДД.ММ.ГГГГ',
        recurring_Failure_singleTariff: 'Оплату по тарифу {1} в рамках счета [{startPeriod};{endPeriod}] выполнить не удалось.',
        recurring_Failure_multiTariff: 'Автоплатеж по счету [{startPeriod};{endPeriod}] выполнить не удалось.',
        ...
        expireTariffPurchase: 'Срок действия тарифа {тариф} истекает ДД.ММ.ГГГГ. Для продолжения пользования тарифом необходимо оплатить счет №(номер счета)',
        expireTariffPurchaseToday: 'Срок действия тарифа {тариф} истекает сегодня',
        expireTariffPurchaseYesterday: 'Тариф {тариф} отключен',
        singleBBill: 'Выставлен счет №... на сумму ... валюта .... Требуется оплатить',
        multipleBBill: 'Задолженность по неоплаченным счетам составляет (сумма) (валюта)',
        singleFBill: 'Для продолжения использования тарифа Премиум необходимо оплатить счет №... на сумму ... валюта ....',
