...
/** ---------- Принятие и отклонение запросов пользователей  --------**/
        ...
        requestJoinClubApprovedByOthers: "{{data[0]}} принят в клуб {{data[2]}}",
        ...
        data[2] - Название клуба 
        data[3] - clubUri  Для ссылки на профиль **/
        requestJoinClubDeclinedByOthers: "{{data[0]}} не принят в клуб {{data[2]}}. Запрос отклонен",

        /** Запрос на выход тренера из клуба ОДОБРЕН другими администраторами. 
         * Отправитель - менеджер, который одобрил запрос
         * Получатели - члены группы clubManagement, за исключением того, кто обработал запрос
        data[0] - Фамилия и Имя инициатора исходного запроса (тренера, который направил запрос клубу)
        data[1] - userUri инициатора запроса
        data[2] - Название клуба 
        data[3] - clubUri  Для ссылки на профиль **/
        requestLeaveClubByCoachApprovedByOthers: "Тренер {{data[0]}} по его просьбе исключен из клуба {{data[2]}}",
        ...
        data[2] - Название клуба 
        data[3] - clubUri  Для ссылки на профиль **/
        requestLeaveClubByCoachDeclinedByOthers: "Тренер {{data[0]}} не исключен из клуба {{data[2]}}. Запрос на выход из клуба отклонен",
        ...
        requestAddFriendApproved: "Ваш запрос на дружбу принят. ",
...
/** ---------- Отключение от тренеров, выход из клубов, выход из групп  --------**/
        /** Уведомление тренеру о том, что отключился спортсмен 
        Отправитель - спортсмен.
        **/
        leaveCoachByAthlete: "Спортсмен отключился от вас",
