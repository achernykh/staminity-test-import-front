...
export class SocketService implements ISocketService {
  ...
  response(event: any, WS: any = this) {
    ...
    /*
     Temporary fix because of backend error
     */
    if (response.data && Array.isArray(response.data) && response.data.length) {
      if (response.data[0].status === 'error' && response.data[0].title && !response['errorMessage']) {
        response['errorMessage'] = response.data[0].title;
      }
    }
