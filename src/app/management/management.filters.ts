...
export const membersFiltersFilter = ($translate) => (filterParams: MembersFilterParams, membersList: MembersList): string => {
    return (
        ...
        filterParams.coachUserId && $translate.instant("users.filters.coach") + ": " + (({ firstName, lastName }) => lastName + firstName.substr(0,1) + '.') (membersList.getMember(filterParams.coachUserId).userProfile.public)
    ) || (
