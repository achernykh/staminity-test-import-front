...
export const membersOrderings: {
    ...
    username: (member) => `${member.userProfile.public.lastName} ${member.userProfile.public.firstName}`,
...
export const getRows = (management: MembersList, filterParams: MembersFilterParams, order: string): Member[] => {
    const rows = management.members.filter(filtersToPredicate(membersFilters, filterParams));
